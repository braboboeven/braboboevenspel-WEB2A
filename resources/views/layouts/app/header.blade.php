    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:navbar class="-mb-px max-lg:hidden">
                ...
                <flux:navbar.item icon="layout-grid" :href="route('spel')" :current="request()->routeIs('spel')" wire:navigate>
                    {{ __('Spel') }}
                </flux:navbar.item>
                @if (auth()->user()?->is_docent)
                    <flux:navbar.item icon="layout-grid" :href="route('docent')" :current="request()->routeIs('docent')" wire:navigate>
                        {{ __('Docent') }}
                    </flux:navbar.item>
                @endif
            </flux:navbar>
